...
# Default to release notes in reStructuredText format
RNFMT=rst
# Uncomment to generate release notes in plain text format
#RNFMT=txt
RNOUT=/tmp/linux_rn.$RNFMT
...
if [ -f "$1" ]; then
	...
	rnoutall=`basename ${PRFILE%.*}`-rn.$RNFMT
	echo "# Release notes auto-generated by 'b4 rn'" > $rnoutall
	cat "$PRFILE" | while read pr name; do
		echo "Writing release notes of '$name' to $rnoutall..."
		>"$name".$RNFMT 2>"$name".log </dev/null \
