...
def run_lore_worker(
    ...
    **kwargs: Any,
    ...
    """Launch a threaded lore fetch with the right worker settings.

    The single sanctioned way to start a lore fetch in a worker thread.  It
    bundles the two things every such fetch needs, so a new site cannot
    ...
    * keeps a fetch failure from tearing down the whole TUI via
    ...
    Cancellation needs no setup here: liblore 0.9 scopes cancellation to
    the operations in flight when ``cancel_active()``/``shutdown()`` is
    called, so a new fetch cannot inherit a stale cancel from a previously
    aborted one.
    """
...
class LoreNodeShutdownMixin:
    """App mixin that shuts the shared lore node down on app exit.
    ...
    parked in its fetch.  Shutting the shared lore node down here makes
    the in-flight request raise ``OperationCancelledError`` promptly, so
    the worker unwinds and the app exits without waiting on the network.
    Unlike a plain ``cancel_active()``, ``shutdown()`` also refuses any
    fetch a racing worker starts *after* this point, so teardown cannot
    be stalled by a straggler opening a fresh connection.

    ``shutdown()`` is terminal for the node, not for the process: when a
    sibling TUI app starts afterwards (e.g. switching between the
    tracking and review apps), ``b4.get_lore_node()`` notices the node is
    shut down and builds a fresh one.
