...
SYNOPSIS
--------
b4 {mbox,am,shazam,pr,diff,ty,kr,dig,review,prep,send,trailers} [options]
...
``b4 review list [--all-projects] [--status STATUS] [-j]``
  List the tracked series, with ``-j`` emitting JSON suitable for
  scripting. Archived series are omitted unless asked for by
  ``--status``. The JSON form includes every message-id known for each
  series, so a script can tell whether a message it found in the
  archives belongs to a series that is already being handled.

``b4 review cleanup [--dry-run]``
  Abandon every tracked series in the ``gone`` state. With ``--dry-run``,
  list the series that would be cleaned up without changing the tracking
  database.
...
``b4 prep --show-cover`` / ``b4 prep --cover-from-file FILE``
  Print the cover letter to stdout, or replace it with the contents of
  FILE (use ``-`` for stdin). Use these instead of ``--edit-cover``
  when there is no human at the keyboard, e.g. from a script.
